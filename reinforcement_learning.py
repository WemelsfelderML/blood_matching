...
def reinforcement_learning(SETTINGS, PARAMS):
    ...
    agent = DQNAgent(SETTINGS, PARAMS, num_bloodgroups, max_request_size)
    if mode == "test":
    ...
    for e in range(SETTINGS.episodes):
        ...
        env = Environment(SETTINGS, PARAMS, num_bloodgroups, max_request_size, e)
        state = env.sample_initial_state(SETTINGS, PARAMS)
        # mean_reward = 0
        ...
        for day in range(SETTINGS.init_days + 1, SETTINGS.init_days + SETTINGS.test_days + 1):
            ...
            action = agent.act(state)
            state_next, reward = env.step(SETTINGS, state, torch.tensor([round(a) for a in action.tolist()]), day)
            # mean_reward = mean_reward + (reward - mean_reward) / day
            agent.remember(torch.Tensor([state]), action, torch.tensor([reward]).unsqueeze(0), torch.Tensor([state_next]))
            state = state_next

        if mode == "train":
            # mean_rewards.append(mean_reward)
            agent.train(SETTINGS.train_iters, e)
            ...
            if e % 5 == 0:
                ...
                model_scripted.save(SETTINGS.generate_filename("log", "nn") + ".pt")  # Save
...
class Environment():
    ...
    def sample_initial_state(self, SETTINGS, PARAMS):
        ...
        for _ in range(PARAMS.max_age):
            inventory_vector += self.sample_supply_single_day(PARAMS, n_products)

        demand_vector = self.sample_requests_single_day(PARAMS)
    ...
    def next_state(self, SETTINGS, inventory_vector, demand_vector, action, day):
        ...
        n_products = max(0, SETTINGS.inventory_size - sum(aging_products))          # Number of products to supply in order to fill complete inventory capacity.
        ...
        # Sample new demand.
        demand_vector = self.sample_requests_single_day(PARAMS, day)

        return inventory_vector + demand_vector


    def sample_supply_single_day(self, PARAMS, n_products):
        ...
        vector = [0 for i in range(2**len(PARAMS.major + PARAMS.minor))]

        # Read each row of the selected dataframe, transform it to a bloodgroup index and add 1 to this index in the supply vector.
        for i in data.index:
            product = Blood(PARAMS, major = vector_to_major([data.loc[i,a] for a in PARAMS.major]), minor = [data.loc[i,a] for a in PARAMS.minor])
    ...
    def sample_requests_single_day(self, PARAMS, day = 0):
        ...
        for n_units in range(self.max_request_size):
            ...
            for i in data_selected.index:
                request = Blood(PARAMS, major = vector_to_major([data_selected.loc[i,a] for a in PARAMS.major]), minor = [data_selected.loc[i,a] for a in PARAMS.minor])
...
class DQNAgent:
    ...
    def __init__(self, SETTINGS, PARAMS, num_bloodgroups, max_request_size):
        # Define DQN Layers
        self.state_vector_length = (num_bloodgroups * PARAMS.max_age) + (num_bloodgroups * max_request_size)
        self.action_vector_length = (num_bloodgroups * PARAMS.max_age)
    ...
    def dqn_load(self, SETTINGS):
        self.dqn = torch.jit.load(SETTINGS.generate_filename("log", "nn") + ".pt")
        self.dqn.eval()
